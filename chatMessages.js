...
if (msg.type === "gifs" || msg.type === "stickers") {
  ...
  contentHtml = `<div class="msg-text">${msg.text.replace(/\n/g, "<br>")}</div>`;
} else {
  // 🚫 Fallback
...
if (msg.text && /^[\p{Emoji}\s]+$/u.test(msg.text.trim())) {
  ...
  messagesDiv.appendChild(el);
  messagesDiv.scrollTop = messagesDiv.scrollHeight;
el.classList.add("fade-in"); // ✅ Εφέ εμφάνισης στο μήνυμα
...
if (clearBtn) {
  clearBtn.addEventListener("click", async () => {
    if (!isAdmin()) {
      showConvoAlert("⛔ Μόνο για Admins.");
      return;
    }
    ...
    try {
      ...
      showConvoAlert(`✅ Τα μηνύματα στο room "${currentRoom}" διαγράφηκαν.`);
    } catch (err) {
