...
clearGuestsBtn?.addEventListener("click", async () => {
  const res = confirm("🧹 Να διαγραφούν όλοι οι offline Guest χρήστες από /status;");
  if (!res) return;
  ...
  try {
    ...
    if (snap.exists()) {
      ...
      snap.forEach((child) => {
        ...
        const isGuest =
          (data.displayName || "Guest") === "Guest" || data.isAnonymous === true;
        const isOffline = (data.state || "offline") !== "online";

        if (isGuest && isOffline) {
          removals.push(remove(ref(db, `status/${child.key}`)));
        }
      });
    ...
    alert(`✅ Διαγράφηκαν ${count} offline Guests από /status.`);
  } catch (err) {
    console.error("❌ Error clearing guests:", err);
    alert("⚠️ Κάτι πήγε στραβά — δες κονσόλα.");
  }
});
