...
function startWatchingMessages(roomId) {
  ...
  off(messagesRef); // 🧹 Καθάρισε προηγούμενο listener (safety)
...
off(messagesRef); // 🧹 Καθάρισε οποιονδήποτε προηγούμενο listener
...
async function showVideo(videoId) {
    if (videoId === currentVideoId) return; // 🧩 Μην ξαναφορτώνεις το ίδιο video
  if (!videoId) return;
